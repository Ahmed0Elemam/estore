...
var userid = GetURLParameter('user_id');
         ...
         function scanToLocalDisk() {
             ...
             var path = "${SCAN_HOME}\\"+userid+"\\"+'M_'+musadqa_id+"\\${TMS}${EXT}";
         if(extention == 1) {
             var ext = "jpg";
         } else {
             var ext = "pdf";
 ...
 <body>
     ...
     if(!file_exists('scanned/'.$user_id.'/'.'M_'.$musadqa_id.'')){
             mkdir('scanned/'.$user_id.'/'.'M_'.$musadqa_id.'', 0777, true);
         }
       } else {
         if(!file_exists('scanned/'.$user_id.'/'.'M_'.$musadqa_id.'')){
             mkdir('scanned/'.$user_id.'/'.'M_'.$musadqa_id.'', 0777, true);
 ...
 <?php foreach ($results as $result) {
     ?>
             ...
             <?php 
             $stmt5 = $connect2->prepare("SELECT * FROM MUSADQA where MUSADQA_ID = ?");
             $stmt5->execute(array($musadqa_id));
 ...
 <div class="row">
         ...
         <br/>
             ...
             <button type="submit" id="move3" class="btn btn-info" type="button">أرشفة الملفات</button>
 ...
 <h4>رفع ملفات</h4>
        ...
        <div class="col">
        <input name="files3[]" type="file" id="files3" class="form-control"  multiple="multiple" />
        <input name="user" type="hidden" id="user" class="form-control" value="<?php echo $user_id; ?>" />
        <input name="musadqa_id" type="hidden" id="musadqa_id" class="form-control" value="<?php echo $musadqa_id; ?>" />
