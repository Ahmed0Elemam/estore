...
$target_path = 'scanned/'.$userid.'/'.$treatment_type.'_'.$department.'_'.$treatment_year.'_'.$treatment_id.'/'; //Declaring Path for uploaded images
    ...
    for ($i = 0; $i < count($_FILES['files_g']['name']); $i++) {//loop to get individual element from the array
        ...
        $ext = explode('.', basename($_FILES['files_g']['name'][$i]));//explode file name from dot(.) 
    ...
    $j = $j + 1;//increment the number of uploaded images according to the files in array       
	  ...
	  if ( in_array($file_extension, $validextensions)) {
            if (move_uploaded_file($_FILES['files_g']['tmp_name'][$i], $target_path.basename($_FILES['files_g']['name'][$i]))) {//if file moved to uploads folder
                $uploaded = true;
                echo  '<div style="font-weight:bold;margin:10px auto;" class="col-md-2 alert alert-success"> تم رفع ';
                echo '<li class="list-unstyled">'.basename($_FILES['files_g']['name'][$i]).'</li>
                <hr/>
                <button class="btn btn-danger delete_only_g" >حذف</button>
            ...
            echo  '<div style="font-weight:bold" class="col-md-2 alert alert-danger">صيغة الملف '.basename($_FILES['files_g']['name'][$i]).' غير مسموحة لم يتم رفعها الرجاء اعادة رفع الملف الصحيح  مرة أخرى</div>';
