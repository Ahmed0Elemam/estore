...
function move_files($destination) {
  ...
  if (!$conn) {
      ...
      $sql ="begin :val:= PKG_ATTACHMENTS.update_not_in_out_treatment(:treatment_id, :treatment_year, :department, :treatment_category, :file, :destination, :user_id); end;";
  ...
  echo "<strong><p class='alert alert-success text-center'>تمت أرشفة الملفات المسحوبة / المرفوعة بنجاح</p></strong>";
  ...
  foreach ($delete as $file) {
    unlink($file);
        ...
        echo "<strong><p class='alert alert-danger text-center'>لم يتم أرشفة الملفات ... الرجاء اعادة السحب/الرفع والأرشفة مرة أخرى</p></strong>";
...
function is_dir_empty($dir) {
  ...
  if (is_dir_empty($dir)) {
    echo "<strong><p class='alert alert-danger text-center'>لم يتم سحب أي ملفات</p></strong>"; 
  }else {
