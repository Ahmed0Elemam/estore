...
2 => "مؤقتة"
 ...
 // Insert to DB
      $stmt2 = $connect->prepare("UPDATE `building_info` SET 
      `building_name`= ?,
      `building_address`= ?,
      `building_tele`=?,
      `building_mobile`=?,
      ...
      `notes`=?
      Where building_code = ?");
        $stmt2->execute(array($building_name,
                                 $building_address,
                                 $building_tele,
                                 $building_mobile,
                                 $building_fax,
            ...
            echo "<div style='margin-top:10px;font-weight:700;font-size:20px' class='alert alert-success'>تم تعديل بيانات المنشأة بنجاح </div>";
