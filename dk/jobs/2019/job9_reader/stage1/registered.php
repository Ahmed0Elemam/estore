...
2 => "مؤهل فوق متوسط",
     ...
     $formErrors = array();
            ...
            if ((substr($n_id,1,2) < 90 and substr($n_id,0,1) == 2) or substr($birthdate, 0, 4) < 1990){
                $formErrors[] = "سنة الميلاد لابد الا تقل عن سنة 1990";
              }
...
<head>
    ...
    <meta property="og:title"  content="اعلان رقم 9  لسنة 2019 | وظيفة محصل بنظام العمولة" /> 
    ...
    <title>اعلان رقم 9 لسنة 2019 | وظيفة محصل بنظام العمولة</title>
...
<body>
    ...
    if (empty($formErrors)) {
    $check = checkItem('national_id','ad9_2019_reader',$n_id);
     
    
      if($check > 0){
                            $msg = "<div class='alert alert-danger inst'>عفوا !!!  تم تسجيل هذه البيانات من قبل !!!</div>";
                            echo $msg;
          $stmt = $connect->prepare("select * from ad9_2019_reader where national_id = $n_id");
          $stmt->execute();
          $rows = $stmt->fetchAll();
          foreach($rows as $row){
          ?>
...
<div class='alert alert-warning' style="font-size:12px;"> تاريخ التسجيل: <?php echo $row['registered_date']; ?> / وقت التسجيل: <?php echo $row['time'];  ?></div>
            ...
            <?php
                            ...
                            INSERT INTO 
                            ad9_2019_reader(name, national_id, birthdate, military, marital, village, street, email, mobile, landline, study, grade_year, study_name, registered_date, time) 
                            VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now(), now())
                            ");
                            $stmt->execute(array($name, $n_id, $birthdate, $military, $marital, $village, $street, $email, $mobile, $landline, $study, $grade_year, $study_name));
