...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (strlen($name) < 15) {
    $formErrors[] = "الاسم رباعي لا يمكن ان يكون اقل من 15 حروف";
  }
  if (strlen($n_id) < 14) {
  ...
  if ($study == 0) {
    $formErrors[] = "لم يتم اختيار الشهادة الحاصل عليها";
  }

  if (strlen($grade_year) < 4) {
    $formErrors[] = "سنة الحصول على الشهادة لايمكن أن تكون أقل من 4 أرقام";
  }
  if (empty($grade_year)) {
    $formErrors[] = "سنة الحصول على الشهادة لا يمكن ان تكون فارغة";
  }
  if (!is_numeric($grade_year)) {
    $formErrors[] = "سنة الحصول على الشهادة لا يمكن ان تكون حروفاً";
  }
  if ($grade_year == 0) {
    $formErrors[] = "سنة الحصول على الشهادة لا يمكن ان تكون أصفارا";
  }
...
<body>
    <div class="container text-center">
        ...
        <h4>اعلان رقم (10) لسنة 2019
        </h4>
        <h4>وظيفة عامل حفر وتسليك وتطهير</h4>
        <hr />
        <?php
     // loop throw errors array and display them
        ...
        if (empty($formErrors)) {
          $check = checkItem('national_id', 'ad10_2019_worker', $n_id);


          if ($check > 0) {
            $msg = "<div class='alert alert-danger inst'>عفوا !!!  تم تسجيل هذه البيانات من قبل !!!</div>";
            echo $msg;
            $stmt = $connect->prepare("select * from ad10_2019_worker where national_id = $n_id");
            $stmt->execute();
            $rows = $stmt->fetchAll();
            foreach ($rows as $row) {
              ?>
        <table id="registered" class="table table-bordered">
            ...
            <tr>
                ...
                <td><strong>
                    ...
                    </strong><strong>الرجاء الاحتفاظ بهذا الكود</strong></td>
            </tr>
            <tr>
                <td>الاسم</td>
                <td>
            ...
            <tr>
                <td>الشهادة الحاصل عليها المتقدم</td>
                <td>
      ...
      $stmt = $connect->prepare("
                            INSERT INTO 
                            ad10_2019_worker(name, national_id, birthdate, military, marital, city, village, street, email, mobile, landline, study,grade_year, registered_date, time) 
                            VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now(), now())
                            ");
      $stmt->execute(array($name, $n_id, $birthdate, $military, $marital, $city, $village, $street, $email, $mobile, $landline, $study, $grade_year));
