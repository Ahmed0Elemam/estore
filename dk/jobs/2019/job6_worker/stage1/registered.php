...
<body>
    <div class="container text-center">
     ...
     // loop throw errors array and display them
        ...
        if (empty($formErrors)) {
          $check = checkItem('national_id', 'ad6_2019_worker', $n_id);


          if ($check > 0) {
            $msg = "<div class='alert alert-danger inst'>عفوا !!!  تم تسجيل هذه البيانات من قبل !!!</div>";
            echo $msg;
            $stmt = $connect->prepare("select * from ad6_2019_worker where national_id = $n_id");
            $stmt->execute();
            $rows = $stmt->fetchAll();
            foreach ($rows as $row) {
              ?>
      ...
      $stmt = $connect->prepare("
                            INSERT INTO 
                            ad6_2019_worker(name, national_id, birthdate, military, marital, city, village, street, email, mobile, landline, study,grade_year, registered_date, time) 
                            VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now(), now())
                            ");
