...
<body>
    <div class="container text-center">
        ...
        <h4>اعلان رقم (7) لسنة 2019
        </h4>
        <h4>وظيفة فني رابع</h4>
        <hr />
        <?php
     // loop throw errors array and display them
        ...
        if (empty($formErrors)) {
          $check = checkItem('national_id', 'ad7_2019_technical', $n_id);


          if ($check > 0) {
            $msg = "<div class='alert alert-danger inst'>عفوا !!!  تم تسجيل هذه البيانات من قبل !!!</div>";
            echo $msg;
            $stmt = $connect->prepare("select * from ad7_2019_technical where national_id = $n_id");
            $stmt->execute();
            $rows = $stmt->fetchAll();
            foreach ($rows as $row) {
              ?>
      ...
      $stmt = $connect->prepare("
                            INSERT INTO 
                            ad7_2019_technical(name, national_id, birthdate, military, marital, city, village, street, email, mobile, landline, study, grade_year, registered_date, time) 
                            VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now(), now())
                            ");
