...
$sqlResult = $connect->prepare("select * from ad8_2019_3omla_stage4
...
//$sqlResult = mysql_query($sql, $link) or die(mysql_error());
$sqlResult2 = $connect->prepare("select * from ad8_2019_3omla_stage4
...
if ($q != -1) {
	...
	if (sizeof($result_data) <= 0) {
		...
		foreach ($result_data as $row) {
			if (is_numeric($row[$degree]) and $row[$degree] >= 35 and $row[$degree] != NULL) {
				$result = "<strong class='text-success'>
                    ناجح
                    </strong>";
			} elseif (is_numeric($row[$degree]) and $row[$degree] < 35 and $row[$degree] != NULL) {
				$result = "<strong class='text-danger'>   
                    لم يتجاوز الحد الأدنى المطلوب
                    </strong>";
			} elseif ( $row[$degree] == NULL) {
				$result = "<strong class='text-danger'>
                  تغيب عن حضور الإختبار التحريري
                  </strong>";
			} else {
				$result = "<strong class='text-danger'>".$row[$degree]."</strong>";
			}
			echo "<tr>";
