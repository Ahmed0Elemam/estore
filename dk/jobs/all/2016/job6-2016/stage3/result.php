...
//database query
	...
	$col_address_city = 'city_name';
	$col_address_village = 'village';
...
?>
  ...
  <body>
    <div class="container text-center">
      ...
      <h3>
        نتيجة استيفاء الأوراق ومواعيد الاختبار التحريري للمتقدمين للاعلان رقم (6) لسنة 2016 </h3>
	...
	if($q != -1) {
		...
		if( sizeof($result_data) <= 0){
			...
			echo		"<th class='text-center'>الاســـم</th>
						...
						<th class='text-center'>نتيجة استيفاء الآوراق</th>
						<th class='text-center'>ميعاد الاختبار التحريري</th>
						<th class='text-center'>التفاصيل</th>
					</tr>";
			foreach($result_data as $row){
				echo "<tr>";
				if($code==0) echo "<td>{$row[$col_id]}</td>";
				...
				echo "<td><a href='details.php?{$col_id}={$row[$col_id]}'>المزيد..</a></td>
					 </tr>";
