...
<body>
    <div class="container text-center">
     ...
     // loop throw errors array and display them
        ...
        if (empty($formErrors)) {
          $check = checkItem('national_id', 'job1_2020_technical', $n_id);


          if ($check > 0) {
            $msg = "<div class='alert alert-danger inst'>عفوا !!!  تم تسجيل هذه البيانات من قبل !!!</div>";
            echo $msg;
            $stmt = $connect->prepare("select * from job1_2020_technical where national_id = $n_id");
            $stmt->execute();
            $rows = $stmt->fetchAll();
            foreach ($rows as $row) {
              ?>
<div class='alert alert-warning' style="font-size:12px;"> تاريخ التسجيل: <?php echo $row['registered_date']; ?> / وقت التسجيل: <?php echo $row['time'];  ?></div>
      ...
      $stmt = $connect->prepare("
                            INSERT INTO 
                            job1_2020_technical(name, national_id, birthdate, military, marital, city, village, street, email, mobile, landline, study, grade_year, registered_date, time) 
                            VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now(), now())
                            ");
