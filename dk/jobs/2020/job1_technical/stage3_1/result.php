...
$sqlResult = $connect->prepare("select * from job1_2020_technical_stage3_paper_result_1
...
$sqlResult2 = $connect->prepare("select * from job1_2020_technical_stage3_paper_result_1
...
$adv_title = "نتيجة استيفاء الأوراق ومواعيد الإختبار التحريري لوظيفة فني رابع | اعلان رقم 1 لسنة 2020";
$adv_link = "dkwasc.com.eg/jobs/2020/job1_2020_technical/stage3_1/";
$job_no = " نتيجة استيفاء الأوراق ومواعيد الإختبار التحريري للمتقدمين للاعلان رقم 1 لسنة 2020";
			...
			echo	"<td><a href='details.php?{$id}={$row[$id]}'>$row[$name]</a></td>
						 <td>$row[$city]</td>";
								 if ($row[$paper_result] == 1) {
									 echo "<td class='text-success'>مستوفي</td>";
									 echo "
									 <td>$row[$exam_day]
						...
						$row[$exam_hour]  </td>";
								 } elseif($row[$paper_result] == 2 or $row[$paper_result] == 3) {
