...
$sqlResult = $connect->prepare("select * from job1_2020_technical_stage4_1_final
...
//$sqlResult = mysql_query($sql, $link) or die(mysql_error());
$sqlResult2 = $connect->prepare("select * from job1_2020_technical_stage4_1_final
	...
	if (sizeof($result_data) <= 0) {
		...
		foreach ($result_data as $row) {
			...
			echo	"<td><a href='details.php?{$id}={$row[$id]}'>{$row[$name]}</a></td>
						 <td>{$row[$city]}</td>";
                         if (is_numeric($row[$ordering])) {
                             echo "<td class='text-success'>$row[$ordering]</td>";
                         } else {
													echo "<td class='text-danger'>$row[$ordering]</td>";
												 }
