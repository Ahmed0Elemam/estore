...
$sqlResult = $connect->prepare("select * from ad6_2018_mengineer_stage5
...
//$sqlResult = mysql_query($sql, $link) or die(mysql_error());
$sqlResult2 = $connect->prepare("select * from ad6_2018_mengineer_stage5
...
if ($q != -1) {
	...
	if (sizeof($result_data) <= 0) {
		...
		foreach ($result_data as $row) {
			if (is_numeric($row[$degree]) and $row[$degree] >= 35 and $row[$degree] != 0) {
				$result = "<strong class='text-success'>
                    ناجح
                    </strong>";
			} elseif (is_numeric($row[$degree]) and $row[$degree] < 35 and $row[$degree] != 0) {
				$result = "<strong class='text-danger'>   
                    لم يتجاوز الحد الأدنى المطلوب
                    </strong>";
			} elseif (is_numeric($row[$degree]) and $row[$degree] == 0) {
				$result = "<strong class='text-danger'>
