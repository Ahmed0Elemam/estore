...
">",
                searching: true,
                    ...
                    $(win.document.body).css('font-size','14px')
                        .prepend(
                            '<h3 class="ui header"> تقرير الغياب لموظفي شركة مياه الشرب والصرف الصحي بالدقهلية</h3>'+
...
<table class="ui celled padded table display center aligned" id="report">
  <thead>
    <tr>
      ...
      <th scope="col"> تاريخ ووقت الامضاء الغير مسموح</th>
...
$attend_limit = date($day_before.' 08:30:00');
$leave_begin = date($day_before.' 14:40:00');

$stmt = $conn->prepare("select * from RAW_TRANSACTIONS as trans join EMPLOYEE_WORK_SCH_GRP as grp on trans.EMP_ID = grp.EMP_ID where trans.TRANS_TIME  > ? AND trans.TRANS_TIME  < ? and grp.GRP_ID = ? and grp.TODATE >= ?");
...
$attend_limit = date($day_before.' 08:30:00');
$leave_begin = date($day_before.' 14:30:00');
...
$attend_limit = date($day_before.' 08:30:00');
$leave_begin = date($day_before.' 14:00:00');
...
// group 111   9 - 3
        ...
        $attend_limit = date($day_before.' 09:30:00');
        $leave_begin = date($day_before.' 14:40:00');
        ...
        // end group 111
                ...
                if($firstday != 'Sunday') {
                        ...
                        $attend_limit = date($day_before.' 10:30:00');
                        $leave_begin = date($day_before.' 14:40:00');
